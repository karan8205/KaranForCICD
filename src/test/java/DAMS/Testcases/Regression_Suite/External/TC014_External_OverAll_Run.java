	...
	// --------------------------------------------
	  ...
	  if(!Functional_Role.equalsIgnoreCase("Production")) {
		  if(Functional_Role.equalsIgnoreCase("Development")||Functional_Role.equalsIgnoreCase("Development - ATG")) {
				...
				List<String >create_replacement_package_request_id_front = create_replacement_package_request_root_External(input);
				waitForPageLoad(driver);
				Thread.sleep(3000);
				approve_special_cases_Replacement_request(input,create_replacement_package_request_id_front);
				waitForPageLoad(driver);
				Thread.sleep(3000);
				List<String >create_replacement_package_request_id_back = create_replacement_package_request_backend_External(input);
...
//	    h.dropBtn();
	...
	public static List<String> create_replacement_package_request_root_External(HashMap<String, String> input) throws Throwable {
		test.info("Replacement package request");
		logger.info("Replacement package request");
		myreq.select_NewPermission_request();		
...
//		Assert.assertTrue(replacement_package_table_input.get(0).equals(prop.getStatus_pending()));
	...
	public static List<String> create_replacement_package_request_backend_External(HashMap<String, String> input) throws Throwable {
		test.info("Replacement package request");
		logger.info("Replacement package request");
		myreq.select_NewPermission_request();		

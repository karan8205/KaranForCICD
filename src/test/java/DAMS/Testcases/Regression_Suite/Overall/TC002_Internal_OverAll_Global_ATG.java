...
public class TC002_Internal_OverAll_Global_ATG extends BaseClass {
	...
	public static void Overall_Global_ATG_Regression_E2E_Internal(HashMap<String, String> input) throws Throwable {
		ExtentReporterNG.GenarateExcelReport(input,"TC002_Internal_OverAll_Global_ATG","Overall_Global_ATG_Regression_E2E_Internal");
		try {
		System.out.println("\n"+input+"\n");
	...
	// --------------------------------------------
		...
		req.request_Overview();
			...
			AbstractComponents.refresh();
			myreq.select_NewPermission_request();
			newper.raise_Enhance_Right_Request(input);
			Thread.sleep(5000);
			req.request_Overview();

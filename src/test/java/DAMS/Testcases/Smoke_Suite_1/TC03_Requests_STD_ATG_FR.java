...
public class TC03_Requests_STD_ATG_FR extends BaseClass {
	// Creating Functional role request-Internal,External and Supplier
	...
	public static void apply_internal_functional_role_create_test_case(HashMap<String, String> input) throws Throwable {
		...
		String select_user_type = login_and_select_user_ATG(ctx().prop.get_user_type_Internal());
	...
	public static void apply_External_functional_role_create_test_case(HashMap<String, String> input) throws Throwable {
		...
		String select_user_type = login_and_select_user_ATG(ctx().prop.get_user_type_external());
		...
		functional_role_Overview_table_validation(select_user_type);
		test.pass(
				"Request created and in pending status");
		logger.info(
				"Request created and in pending status");
		softAssertionALL();
	}
	...
	public static void apply_supplier_functional_role_create_test_case(HashMap<String, String> input) throws Throwable {
		...
		String select_user_type = login_and_select_user_ATG(ctx().prop.get_User_type_Supplier());
		...
		functional_role_Overview_table_validation(select_user_type);
		// ctx().h.click_here_to_apply_for_Functional_roles_disabled();
		test.pass(
				"Request created and in pending status");
		logger.info(
				"Request created and in pending status");
	...
	public static List<Object> functional_role_Overview_table_validation(String select_user_type) throws Throwable {
		...
		List<Object> functional_role_Overview_table_validation = ctx().f.functional_role_Overview_table_validation("Myself",
				ctx().prop.getUser_name(), select_user_type);
		softassertTrue(s, (boolean) functional_role_Overview_table_validation.get(0).equals(ctx().prop.getStatus_pending()));
		test.pass("View the approval status as pending once the request created");
		logger.info("View the approval status as pending once the request created");
		test.log(Status.INFO, "Requester details : " + "<span style=\"color: blue;\"><b><i><u>"
				+ functional_role_Overview_table_validation + "</u></i></b>");
		logger.info("Requester details : " + "<span style=\"color: blue;\"><b><i><u>"
				+ functional_role_Overview_table_validation + "</u></i></b>");
	...
	public static String login_and_select_user_ATG(String user_type) throws Throwable {
		...
		ctx().newrequest.select_vehicle_program_ATG(user_type);
		ctx().newrequest.functional_role_page_validation(ctx().prop.get_for_whom_txt(), ctx().prop.get_myself_txt(),
				ctx().prop.get_Functional_role_txt());
